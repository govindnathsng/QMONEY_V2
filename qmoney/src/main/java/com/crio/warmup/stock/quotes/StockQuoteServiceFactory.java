...
public enum StockQuoteServiceFactory {
  ...
  public StockQuotesService getService(String provider,  RestTemplate restTemplate) {
    ...
    if ("tiingo".equals(lowercaseProvider)) {
    ...
    // else {
    //   // Handle the case where an unsupported provider is provided
    //   throw new IllegalArgumentException("Unsupported provider: " + provider);
    //     }
      } catch (Exception e) {
        // Handle any exceptions that might occur during service instantiation
        throw new RuntimeException("Error creating StockQuotesService", e);
      }
    }
  }
